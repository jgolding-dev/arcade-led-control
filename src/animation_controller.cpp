...
// Array (vector) of unique_ptr to the Parent type
std::vector<Zone*> LED_ZONES;

    // When we iterate, the child's version of the function is called
    // for (const auto& animal : farm) {
    //     animal->makeSound(); // Polished call
    // }

// Full* full_zone = new Full(BRIGHTNESS_MAX);
// Player1* player1_zone = new Player1(BRIGHTNESS_MAX);
// Player2* player2_zone = new Player2(BRIGHTNESS_MAX);
// Options* options_zone = new Options(BRIGHTNESS_MAX);
// Backlight* backlight_zone = new Backlight(BRIGHTNESS_MAX);
